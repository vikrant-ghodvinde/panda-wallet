...
import {
  ...
  H_SWIPE_RANGE,
  H_WAVE_RANGE,
  SWIPABLE_THUMB_LIGHT,
  SWIPABLE_BACKGROUND_LIGHT,
} from "./Constants";
import { ArrowRight, ChevronRight } from "../../../icons";

const SlidableButtonLight = ({ label, onPress }) => {
    ...
    <GestureHandlerRootView className="w-full">
      <View style={styles.swipeContainer}>
        ...
        <Animated.View style={[styles.swipeText, animatedStyles.swipeText]}>
          <Text className="text-black font-urbanist-medium capitalize tracking-wider">{label}</Text>
          <View className="absolute right-5 flex flex-row">
            <ChevronRight size={22} color="#c2c2c2" />
            <ChevronRight size={26} color="#3b333c" />
...
const styles = StyleSheet.create({
  swipeContainer: {
    ...
    backgroundColor: SWIPABLE_BACKGROUND_LIGHT,
    display: "flex",
    alignItems: "center",
  ...
  swipable: {
    ...
    backgroundColor: SWIPABLE_THUMB_LIGHT,
    position: "absolute",
    left: BUTTON_PADDING,
    display: "flex",
    alignItems: "center",
...
export default SlidableButtonLight;
